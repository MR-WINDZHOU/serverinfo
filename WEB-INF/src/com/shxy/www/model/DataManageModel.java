...
public class DataManageModel {
	 ...
	 * @throws Exception
	 */
	public Map<String, String> saveSqliteInfo(Map<String, String> strMap) throws Exception{
		Hashtable<String, String> result = new Hashtable<String, String>();
		if(!MapUtil.checkMapParams(strMap)){
			result.put("result", "0");
			return result;
		}
		SQLAssemblyUtil sqlUtil = SQLAssemblyUtil.factory(this.dbManager);
		String sql = sqlUtil.map2SQL(strMap);//拼接SQL脚步
		if(sql.equals("")){//如果脚步为空，直接返回
			result.put("result", "0");
			return result;
		}
		//如果参数中包含ID，表示更新，否则是添加
		if(strMap.containsKey("ID")){//更新
			if(this.dbManager.executeUpdateSQL(sql)>0){
				result.put("result", "1");
				return result;
