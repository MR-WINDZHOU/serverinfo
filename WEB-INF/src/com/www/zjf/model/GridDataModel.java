...
public class GridDataModel {
	...
	public Map<String, Object> gridData(Map<String, String> strMap) throws Exception{
		...
		case 2://转换类型列表信息
			...
			break;
		case 4://报表中已配置的字段信息
			result = this.getServerInforList(strMap);
			break;
		case 5://报表配置字段详细转换信息
	 ...
	 * @throws Exception
	 */
	private Map<String, Object> getServerInforList(Map<String, String> strMap) throws Exception{
		Map<String, Object> result = new Hashtable<String, Object>();
		String itemid = strMap.get("ITEMID");
		if("".equals(itemid)||itemid==null){
			itemid = strMap.get("ID");
		}
		String sql = "SELECT itemid,colname,datavalue,xuhao FROM serverinfo where itemid="+itemid;
		List<List> ipLists = this.dbManager.executeQueryList(sql);
		List<Map<Object, Object>> recordsMap = new ArrayList<Map<Object, Object>>();
		for(int i=0; i<ipLists.size(); i++){
			Map<Object, Object> singleInfo = new HashMap<Object, Object>();
			singleInfo.put("itemid", ipLists.get(i).get(0));
			singleInfo.put("colname", ipLists.get(i).get(1));
			singleInfo.put("datavalue", ipLists.get(i).get(2));
			singleInfo.put("order", ipLists.get(i).get(3));
			recordsMap.add(singleInfo);
		}
		result.put("list", recordsMap);
		return result;
	...
	private Map<String, Object> tableFields(Map<String, String> strMap) throws Exception{
		...
		String sql = " SELECT companyid,companyname,companystatus,remark from customer";
		List<List> customers = this.dbManager.executeQueryList(sql);
		if(ListUtil.listIsNull(customers)){
			return result;
		}
		List<Map<Object, Object>> recordsMap = new ArrayList<Map<Object, Object>>();
		for(int i=0; i<customers.size(); i++){
			Map<Object, Object> singleInfo = new HashMap<Object, Object>();
			singleInfo.put("companyid", customers.get(i).get(0));
			singleInfo.put("companyname", customers.get(i).get(1));
			singleInfo.put("companystatus", customers.get(i).get(2));
			singleInfo.put("remark", customers.get(i).get(3));
			recordsMap.add(singleInfo);
		}
		result.put("list", recordsMap);
		return result;
